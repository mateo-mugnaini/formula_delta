export function validateTimingEntry(entry) {
  ...
  if (typeof entry.driverId !== 'string' || entry.driverId.length === 0)
    errors.push('driverId must be a non-empty string');
  if (entry.position !== undefined && entry.position !== null && !Number.isInteger(entry.position))
    errors.push('position must be an integer or null');
  if (entry.lapCount !== undefined && entry.lapCount !== null && !Number.isInteger(entry.lapCount))
    errors.push('lapCount must be an integer or null');
